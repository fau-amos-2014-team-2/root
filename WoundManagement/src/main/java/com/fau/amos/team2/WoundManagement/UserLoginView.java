...
public class UserLoginView extends NavigationView {
	...
	public UserLoginView() {
		...
		int buttonValue = 1;
		//fill grid with buttons - values from 1 to 9
		for (int i=0; i<3; i++)
		{
			for (int j=0; j<3; j++)
			{
				Button valueButton = new Button("" + buttonValue);
				final int valButton = buttonValue;
				valueButton.addClickListener(new ClickListener() {
					public void buttonClick(ClickEvent event) {
						passwordField.setValue(passwordField.getValue() + valButton); 

					}
				});			
				valueButton.setSizeFull();
				grid.addComponent(valueButton, j, i);		
				buttonValue++;
			}
		}
		...
		//zeroButton.setClickShortcut(KeyCode.NUM0);
		final int valButton = 0;
		zeroButton.addClickListener(new ClickListener() {
			public void buttonClick(ClickEvent event) {
				passwordField.setValue(passwordField.getValue() + valButton); 

			}
		});	
		grid.addComponent(zeroButton);
		
		Button loginButton = new Button("Login");
		loginButton.addClickListener(new Button.ClickListener() {
			public void buttonClick(ClickEvent event) 
			{
				//final step of user-login process
				validateData();
			}
		});
		loginButton.setClickShortcut(KeyCode.ENTER);
		grid.addComponent(loginButton);
		fieldsandbutton.addComponent(grid);
